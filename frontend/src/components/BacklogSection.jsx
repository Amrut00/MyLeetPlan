    ...
    <div id="backlog-section" className="bg-dark-bg-tertiary rounded-lg shadow-sm border border-dark-border p-3 sm:p-4">
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-xl sm:text-2xl font-bold text-dark-text flex items-center gap-2">
          <HiOutlineClock className="w-6 h-6 text-orange-400" />
          <span>Backlog (Missed Repetitions)</span>
        </h2>
        <span className="px-2 py-0.5 bg-orange-900/30 text-orange-300 rounded-full text-xs font-semibold border border-orange-700/30">
          {problems.length}
        </span>
      </div>
      ...
      <div className="space-y-3">
        {Object.entries(groupedProblems).map(([date, dateProblems]) => (
          <div key={date} className="bg-dark-bg-secondary rounded-lg p-3 sm:p-4 border border-dark-border">
            <h3 className="font-semibold text-dark-text-secondary mb-2.5 text-sm sm:text-base">
              Due: {date}
            </h3>
